...
st.markdown(
    """
    Check whether B2B SaaS companies have an active blog or resources section.

    **Expected CSV format:**
    - `Company Name`
    - `Website`

    Results include the columns:
...
if uploaded_file is not None:
    ...
    if "Company Name" not in rows[0] or "Website" not in rows[0]:
        st.error("The CSV must contain 'Company Name' and 'Website' columns.")
        st.stop()

    st.success(f"**{len(rows)}** companies loaded.")
    with st.expander("Preview"):
        st.dataframe(rows[:10], use_container_width=True)

    col1, col2 = st.columns(2)
    with col1:
        timeout = st.slider("⏱️ Timeout per request (seconds)", 5, 30, 10)
    with col2:
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(future_to_index):
                ...
                progress_bar.progress(
                    completed / len(rows),
                    text=f"Analyzing {completed}/{len(rows)}: {result.company} → {result.verdict}",
                )
                status_text.text(f"Latest: {result.company} → {result.verdict}")
